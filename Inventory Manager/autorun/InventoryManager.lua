...
log.info(string.format(
    "[InventoryManager] Loaded v%s (combat/camp/town gated Pawn retrieval; fail-closed native transfer guard; transfer safety stop; persistent stack reapply after Reset Scripts; %d mutation steps/update).",
    tostring(core.VERSION or "?"),
    core.getMutationStepsPerUpdate()
))
core.debugLog("Basic", "SESSION START version=%s configuredLevel=%s mutationStepsPerUpdate=%d", tostring(core.VERSION or "?"), core.getDebugLogLevel(), core.getMutationStepsPerUpdate())
